...
- `schillerkiez_social` - MSS social index data per PLR area per report period (unique: `plr_code + period`). `dynamik_class` (`positiv|stabil|negativ`) added 2026-09-09 — the Dynamik-Index is a class in the MSS file, and `dynamik_index` had been a silent 0 for every row; prod needs one `Sync Schillerkiez Stats` workflow dispatch to populate it (see `src/components/kiez/CLAUDE.md`).
