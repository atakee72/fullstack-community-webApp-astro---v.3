...
- **Send modules** (`src/lib/auth/send*.ts`, contact relay) own copy + dev-log fallbacks; the mailer owns transport, From (`SENDING_FROM_EMAIL`), timeouts (10s connect, `requireTLS`). `sendWelcomeEmail.ts` fires from `/api/auth/verify-email` on the FIRST `emailVerified` false→true transition only (exactly-once via `findOneAndUpdate` returnDocument 'before'), awaited best-effort inside the request window — never at registration (deliberate: no double mail on signup).
