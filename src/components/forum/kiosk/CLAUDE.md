...
- **Pinned slot on the forum index** is derived client-side: officials with `isOfficial && pinnedUntil > now`, sorted newest pin first, sliced to `MAX_PINS`. Only the first (`i === 0`) renders full-width (`col-span-3`); older pins render as slim one-line pin bars (ink bg, teal border/shadow, `pinned.bar.label` + title + relative time), not full cards — Variant B from the Aug 2026 design canvas. Clicking a bar swaps that pin into the featured slot and collapses the previous one to a bar (client-side view state `expandedPinId`, per-visit only — newest pin is featured again on reload); the featured card keeps its click-through to the detail page. Excluded from `filteredRest` so none appear twice.
