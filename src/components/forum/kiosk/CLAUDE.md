...
- **Pinned slot on the forum index** is derived client-side: officials with `isOfficial && pinnedUntil > now`, sorted newest pin first, sliced to `MAX_PINS`. Pins keep their newest-first positions and ALL start collapsed as slim one-line pin bars (ink bg, teal border/shadow, `pinned.bar.label` + title + relative time), stacked 8px apart in one full-width grid cell — accordion v3 (2026-09-12, superseding the Aug v2 "newest pin expanded as featured card"): clicking a bar opens its full card beneath the bar with a `slide` micro-animation (180ms, 0 under `prefers-reduced-motion`) and collapses any other open one; clicking the open bar (▴, `aria-expanded`/`aria-controls`) closes it (client-side view state `expandedPinId`, per-visit only). The open card is NOT a link: `ForumPostCard` gets `readHref` and renders its "→ lesen" CTA as the one real link (44px tap target); the author name keeps its profile click. Excluded from `filteredRest` so none appear twice.
