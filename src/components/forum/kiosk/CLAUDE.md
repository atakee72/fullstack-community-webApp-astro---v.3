...
- **Pinned slot on the forum index** is derived client-side: officials with `isOfficial && pinnedUntil > now`, sorted newest pin first, sliced to `MAX_PINS`. Only the first (`i === 0`) renders full-width (`col-span-3`); older pins render as slim one-line pin bars (ink bg, teal border/shadow, `pinned.bar.label` + title + relative time), not full cards — Variant B from the Aug 2026 design canvas. Excluded from `filteredRest` so none appear twice.
