...
- **Pinned slot on the forum index** is derived client-side: officials with `isOfficial && pinnedUntil > now`, sorted newest pin first, sliced to `MAX_PINS`. Pins keep their newest-first positions; exactly ONE is expanded in place as the full-width featured card (newest by default), the rest render as slim one-line pin bars (ink bg, teal border/shadow, `pinned.bar.label` + title + relative time) — accordion behavior (Aug 2026 v2, superseding the brief swap-to-top version): clicking a bar expands it in its own row with a `slide` micro-animation (180ms, 0 under `prefers-reduced-motion`) and collapses the open one (client-side view state `expandedPinId`, per-visit only — newest pin expanded again on reload). The expanded card keeps its click-through to the detail page. Excluded from `filteredRest` so none appear twice.
...
- **Likes ARE wired now** (`ForumPostDetail.svelte`, `e0d0c490`). The engagement-strip ♥ button was inert through the whole kiosk rebuild — no handler; `useLikeMutation.ts` (named in the Legacy notes below + root CLAUDE.md) is LEGACY React, never ported. Now a plain-fetch optimistic toggle mirroring `toggleBookmark()`: local `liked`/`likeCount` `$state` (init from `topic.likedBy?.includes(currentUserId)` + `topic.likes`), an **in-flight lock** (the `/api/likes/toggle` `$inc` is NOT conditioned on `$addToSet` actually adding, so never fire a redundant action — a double-send would drift the count), reconcile the count from the response's `likeCount`, rollback + `showError` on failure. Fills wine + `aria-pressed` when liked. **Detail page only** — feed cards + comments still have no like affordance (deferred). Optional follow-up: make the API idempotent (derive `likes` from `likedBy.length`).
