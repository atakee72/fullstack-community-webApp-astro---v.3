...
- **Warning-labeled listings (`hasWarningLabel === true`) are bumpable AND editable.** Warning labels mean approved-with-caveat — the content is publicly visible (blurred until the viewer clicks through). Bumping doesn't change the content, just the freshness timestamp. The bump + status endpoints pass `{ allowOnWarningLabel: true }` to `canMutateListing`; the edit endpoint does too, BUT it ALSO writes a pre-edit snapshot to the `listingAuditTrail` collection before the moderation re-run (provable record of the warning state immediately before the edit). If the fresh re-moderation passes (new content is clean), `hasWarningLabel` + `warningText` are cleared on the same `$set`; `isUserReported` is intentionally NOT cleared (user reports are independent of AI warnings — admin clears them via `/admin/moderation`). The "editing evades the warning" attack isn't real because moderation re-runs on every edit; the audit trail is for legal/regulatory provability of what the warning said + when it was cleared.
