...
<div class="auth-card">
  ...
  {:else}
    ...
    <div style="text-align:center; margin-top:16px;">
      <a href={hasSession ? '/forum' : '/login'} class="font-dmmono no-underline kiosk-tap" style="font-size:11px; color:var(--k-ink-soft); border-bottom:1px dashed var(--k-ink-mute);">{hasSession ? $t['auth.verify.toForum'] : $t['auth.verify.back']}</a>
    </div>
  {/if}
</div>
