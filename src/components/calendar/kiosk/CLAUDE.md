...
**Deferred (ticketed in memory `open-follow-up-tickets-cross-feature`):** (1) ~~deleting an event leaves an orphaned `flaggedContent`/report record~~ — **SHIPPED 2026-09-08**: fixed app-wide (all 6 self-delete handlers, not just events) — the report stays `pending` in the queue marked `contentDeleted: true`, strikeable from the snapshot, with a „Vom Autor gelöscht" admin badge. See root `CLAUDE.md` „User reports". (2) the view-switcher's `aria-label="View"` is a hardcoded untranslated string.
