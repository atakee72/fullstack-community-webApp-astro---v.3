...
**Deferred (ticketed in memory `open-follow-up-tickets-cross-feature`):** (1) deleting an event leaves an orphaned `flaggedContent`/report record — no cascade-clean (parallels the deletion-pipeline sweeps); (2) the view-switcher's `aria-label="View"` is a hardcoded untranslated string.
