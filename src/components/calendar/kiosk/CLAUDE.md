...
- **One mobile header for all three views** (`5f4cfb5f` + follow-up, user-reported): agenda/day used to render the desktop `CalendarTitleBlock` on phones — desktop kicker („KALENDER · AUGUST — OKTOBER"), the week-stats lines, no title, and the stepper/switcher wrapped to the LEFT — while month view shows `CalendarMobileMonth`'s hero (dateline kicker, title, stepper right, „N Termine" left + switcher right). `CalendarTitleBlock` now carries a `lg:hidden` copy of that hero (same dateline derivation off `$now`, props `monthEvents`/`showToday`/`onToday` threaded from `CalendarPageInner`) and keeps its desktop markup under `hidden lg:block`. If the hero changes in one place, change the other — deliberate duplication over a shared component, to avoid touching `CalendarMobileMonth` while the peer's coachmark fix was pending on the same file. Still different by design: the category rail (month = compact colored pills, agenda/day = `CalCategoryRail` with „Alle").
